...
local function sampIsPlayerStreamed(id)
...
local function togglePotato()
    ...
    local candidates = getStreamedCandidates(nil)
    if #candidates < 2 then 
        return sendMsg(C.WARN, "Для запуска нужно минимум 2 игрока в зоне стрима (пешком)!") 
    end

    local function announceWinner(winnerId)
        local winNick = sampGetPlayerNickname(winnerId)
        sampSendChat(string.format("/smp Остался 1 игрок, выиграл %s[%d]", winNick, winnerId))
        sampSendChat(string.format("/gethere %s", winNick))
    ...
    TH.potato = lua_thread.create(function()
        while st.potatoActive do
            ...
            if st.potatoHolder and sampIsPlayerConnected(st.potatoHolder) then
                ...
                if exH and doesCharExist(pHandle) and not isCharDead(pHandle) and not isCharInAnyCar(pHandle) then
                    local myX, myY, myZ = getCharCoordinates(PLAYER_PED)
                    local hx, hy, hz = getCharCoordinates(pHandle)
                    if getDistanceBetweenCoords3d(myX, myY, myZ, hx, hy, hz) <= 1000.0 and math.abs(hx) > 2.0 then
                        holderValid = true
                        curHolderPed = pHandle
                    end
                end
            end

            if not holderValid then
                local nextCand = getStreamedCandidates(nil)
                if #nextCand == 1 then
                    announceWinner(nextCand[1])
                    break
                elseif #nextCand > 1 then
                    ...
                    sampSendChat(string.format("/smp Картошка перешла к %s[%d] (предыдущий покинул зону)!", nNick, st.potatoHolder))
                else
                    sendMsg(C.WARN, "В зоне стрима не осталось игроков! МП завершено.")
                    st.potatoActive = false
                    break
                end
            end
            ...
            if os.clock() >= st.potatoTimer then
                ...
                wait(1500)
                local nextCand = getStreamedCandidates(loserId)
                if #nextCand == 1 then
                    announceWinner(nextCand[1])
                    break
                elseif #nextCand > 1 then
                    ...
                    sampSendChat("/smp Мероприятие окончено! Выживших не осталось.")
                    st.potatoActive = false
                    break
                end
            end
